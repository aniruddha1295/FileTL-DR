...
**Update (2026-09-05):** revisited this scope call after external review flagged the red-band compound decision as never having been exercised live end-to-end (only tests + the scripted demo, per the note above). Rather than an organic multi-week drain, `src/onchain/live-decision-run.ts` passes an explicit `targetRunwayEpochs` into the (already first-class, documented) `DecisionConfig` — a long-horizon (~5yr) archival target — so a real, small testnet deposit's genuine ~334-day runway reads as 18.3% of that target: a real red band from real numbers, without needing to wait or drain further. Two real decisions resulted:
  - Fresh dataset `33859` (proven immediately, real PDP `verified`) → red band → real aggressive top-up, **executed for real**: deposit tx `0x814b84c3fafc5581a904cf24280219bde22c1937357f8cf3ffe336ddcdb1085f`, confirmed calibration block `4043404`.
  - The original `live-verify.ts` dataset (`32848`, terminated 2026-09-02) → a live `RealPDPStatusChecker` read against the *same* account state genuinely comes back `unverified` (its proof simply aged past its challenge window after termination — no faking needed) → same red band, opposite branch → `drop-dataset`. Not re-executed (already terminated); the reason field says so explicitly.

  Both captured in `src/demo/live-run-record.ts` and replayable on the deployed dashboard via a new **"Live Verified Run (real testnet)"** button (`src/ui/page.ts`, wired in `src/demo/run-live-demo.ts`) — real tx hash renders as a real clickable Filfox link via the dashboard's existing `isRealTxHash` check, no UI changes needed for that part. This closes the original gap: the compound red-band decision has now genuinely fired both ways on calibration testnet, not just in tests.

**Tasks**
- [x] Force at least one decision branch against real onchain state (green/none — done above)
- [x] Force the red-band compound decision (both branches: top-up on verified, drop on unverified) against real onchain state — done 2026-09-05, see update above
